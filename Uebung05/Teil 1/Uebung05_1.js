...
// each city gets the new property temperature at the time of the upload
function loadCities(){
        ...
        if(fType == "json" || fType =="geojson"){
            ...
            reader2.onload = async function(){
                ...
                for (let i=0; i<cities.features.length; i++){
                    cities.features[i].properties.temperature = await getWeather(cities.features[i])
...
// This function returns the temperature at a point
// Data is taken from OpenWeatherMap
// Input is a point in the geojson format
